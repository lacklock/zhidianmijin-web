...
function DetailRow({ icon, label, children }: { icon: ReactNode; label: string; children: ReactNode }) {
  return <div className="grid grid-cols-[110px_minmax(0,1fr)] gap-3 py-4 border-b border-border text-sm leading-6 [&_dt]:flex [&_dt]:gap-1.5 [&_dt]:items-start [&_dt]:font-medium [&_dt_svg]:size-4 [&_dt_svg]:mt-1 [&_dd]:m-0 [&_dd]:text-secondary-foreground [&_dd]:text-right [&_dd]:wrap-anywhere max-[850px]:grid-cols-[1fr] max-[850px]:gap-1 max-[850px]:py-3 max-[850px]:[&_dd]:text-left max-[850px]:[&_dd]:pl-5.5"><dt>{icon}{label}</dt><dd>{children}</dd></div>
}
  ...
  return <section aria-labelledby="jobs-heading">
    ...
    <div className="grid grid-cols-[minmax(0,1.7fr)_minmax(0,1fr)] gap-5 items-stretch max-[850px]:grid-cols-[minmax(0,1.25fr)_minmax(0,1fr)] max-[850px]:gap-4">
      <div className="flex flex-col gap-3" aria-label="推荐岗位">{data.jobs.map((job) => <button type="button" key={job.id} className="group relative w-full min-h-29 flex items-center gap-4 p-5 text-left border border-border rounded-lg bg-card transition-colors duration-160 hover:border-primary aria-pressed:border-primary aria-pressed:bg-[#edf6fd] aria-pressed:shadow-[inset_3px_0_0_var(--primary)] max-[850px]:p-4 max-[850px]:gap-3 max-[850px]:min-h-32.5" aria-pressed={selectedJob?.id === job.id} aria-controls="job-details" onClick={() => setSelectedId(job.id)}>
        ...
        <span className="min-w-0 flex flex-col gap-1 pr-2.5"><span className="text-base font-semibold leading-6 wrap-anywhere">{job.title}</span><span className="text-muted-foreground leading-[22px]">{job.company}</span><span className="flex flex-wrap gap-x-4 gap-y-1 mt-1 text-secondary-foreground text-xs leading-[20px] [&_>_span]:inline-flex [&_>_span]:gap-1 [&_>_span]:items-center [&_svg]:size-3.5"><span><MapPin aria-hidden="true" />{job.location}</span><span><GraduationCap aria-hidden="true" />{job.degree}</span></span></span>
