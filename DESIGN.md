...
岗位条件面板顶部「精准匹配」使用 React Bits Grainient 的 WebGL 网格渐变。三色走组件默认值：`#000786`、`#B497CF`、`#00D9FF`，不走设计令牌。标题、图标和说明用白字；关闭浅色模式。WebGL2 不可用时回退为同三色的 CSS 斜向渐变；减少动态效果模式下只渲染静止的一帧。此处为该标题区专用的渐变例外，不要铺到岗位卡片或筛选栏。
